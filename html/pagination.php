...
function pagination_item_active(&$item)
{
	...
	return '<a class="w3-button" title="' . $item->text . '" href="' . $item->link . '" class="pagenav" aria-label="' . $aria . '">' . $display . '</a>';
...
function pagination_item_inactive(&$item)
{
	...
	if ($item->text === JText::_('JLIB_HTML_START'))
	{
		return '<a class="w3-button disabled">&laquo;</a>';
	}

	// Check for "End" item
	if ($item->text === JText::_('JLIB_HTML_END'))
	{
		return '<a class="w3-button disabled">&raquo;</a>';
	}
	...
	if (isset($item->active) && $item->active)
	{
		...
		return '<a class="w3-button active hidden-phone" aria-current="true" aria-label="' . $aria . '">' . $item->text . '</a>';
	}

	// Doesn't match any other condition, render a normal item
	return '<a class="disabled hidden-phone">' . $item->text . '</a>';
}
